...
function setUpPhong(program) {
    ...
    // Light Position: (x, y, z) - Placing it slightly up and to the right
    gl.uniform3f(program.lightPosition, 2.0, 2.0, 5.0);
    
    // Light Color: White light
    gl.uniform3f(program.lightColor, 1.0, 1.0, 1.0);
    
    // Ambient Light: Low intensity white/gray
    gl.uniform3f(program.ambientLight, 0.2, 0.2, 0.2);

    // --- MATERIAL UNIFORMS (Red Plastic look) ---
    
    // Base Color (Diffuse color of the object): Red
    gl.uniform3f(program.baseColor, 0.8, 0.0, 0.0);
    
    // Specular Highlight Color: White reflection
    gl.uniform3f(program.specHighlightColor, 1.0, 1.0, 1.0);
    
    // Coefficients (Phong model weights)
    gl.uniform1f(program.ka, 0.4); // Ambient reflection coefficient
    gl.uniform1f(program.kd, 0.6); // Diffuse reflection coefficient
    gl.uniform1f(program.ks, 1.0); // Specular reflection coefficient
    
    // Shininess (ke): Higher values = smaller, tighter highlight
...
function setUpCamera(program) {
    ...
    glMatrix.mat4.perspective(projMatrix, 45 * Math.PI / 180, gl.canvas.width / gl.canvas.height, 0.1, 100.0);
    gl.uniformMatrix4fv(program.uProjT, false, projMatrix);

    // 2. View Matrix (Camera)
    // Eye at (0, 0, 5) looking at Center (0, 0, 0) with Up (0, 1, 0)
